...
<!-- Detalle -->
	<xsl:template match="Detalle">
		...
		<!-- Fila especial para referencias -->
		<xsl:for-each select="../Referencia">
			<fo:table-row>
					...
					<fo:block font-size="8pt" color="black">
						...
						<fo:block>
							<xsl:call-template name="splitLines">
								<xsl:with-param name="text" select="../Detalle/DscItem"/>
							</xsl:call-template>
						</fo:block>
						<!-- Línea 2: Nombre de documento según TpoDocRef, FolioRef y FchRef -->
						<fo:block>
							<xsl:choose>
								<xsl:when test="TpoDocRef='33'">Factura Electrónica</xsl:when>
								<xsl:when test="TpoDocRef='34'">Factura Exenta Electrónica</xsl:when>
								...
								<xsl:otherwise>Documento Referencia</xsl:otherwise>
							</xsl:choose>
							<xsl:text> [</xsl:text>
						...
						<!-- Línea 4: IT1 del TED -->
						<fo:block>
							<xsl:value-of select="../TED/DD/IT1"/>
						</fo:block>
					</fo:block>
				</fo:table-cell>
				<fo:table-cell text-align="right" border-left-width="0.5pt"
				...
				<fo:table-cell text-align="right" border-left-width="0.5pt"
					border-left-style="solid" border-right-style="solid" border-right-width="0.5pt"
					margin-right="2mm" height="0.8cm">
					<fo:block/>
				</fo:table-cell>
			</fo:table-row>
		</xsl:for-each>
